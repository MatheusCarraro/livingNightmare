...
var gamygdalaEngine = new TUDelft.Gamygdala()

// Primeiro é criado o agente para o jogador
var jogador = gamygdalaEngine.createAgent('jogador')

// Instancia as variáveis globais
var coelhos = 0, ursos = 0, elefantes = 0
var coelho = new Object, urso = new Object, elefante = new Object

class CoelhosZumbi {
	constructor(id) {
		this.id = id
		this.agente = this.criaAgente();
		this.criaMetas();
	}
	
	criaAgente () {
		return gamygdalaEngine.createAgent(this.id)
	...
	criaMetas() {
		...
		gamygdalaEngine.createGoalForAgent(this.id, 'ursoMorrerCoelho', 0.5, true)
		gamygdalaEngine.createGoalForAgent(this.id, 'ficarVivo'+this.id, 0.75, false)
	}

	serAtacado () {
		gamygdalaEngine.appraiseBelief(1, 'jogador', ['jogadorPontuar', 'ficarVivo'+this.id], [0.5, -0.5], true)
	}
	// atacouJogador (idAtacante) {
	// 	gamygdalaEngine.appraiseBelief(0.7, idAtacante, ['matarJogador'], [0.75], true)
	// }

	// vidaJogador30 (idAtacante) {
	// 	gamygdalaEngine.appraiseBelief(0.5, idAtacante, ['matarJogador'], [1], false)
	// }
}
...
class ElefanteZumbi {
	...
	criaMetas() {
		gamygdalaEngine.createGoalForAgent(this.id, 'coelhoMorrerElefante', -0.75, true)
		gamygdalaEngine.createGoalForAgent(this.id, 'ursoMorrerElefante', -0.75, true)
		gamygdalaEngine.createGoalForAgent(this.id, 'ficarVivo'+this.id, -0.75, false)
	}
...
function criaAgenteCoelho () {
	coelho[id] = new CoelhosZumbi(id);
	coelhos = 1;
	return `Criei o ${id}`;
}

function criaAgenteUrso () {
	urso[id] = new UrsoZumbi(id);
	ursos = 1;
	return `Criei o ${id}`;
}

function criaAgenteElefante () {
	elefante[id] = new ElefanteZumbi(id);
	elefantes = 1;
	return `Criei o ${id}`;
}


function appraise(crenca, agente) {
	switch (agente) {
		case "coelho":
			if (crenca == "serAtacado")
				coelho[id].serAtacado();
			if (crenca == "atacouJogador")
				gamygdalaEngine.appraiseBelief(0.7, id, ['matarJogador'], [0.75], true)
			if (crenca == "vidaJogador30")
				gamygdalaEngine.appraiseBelief(0.5, id, ['matarJogador'], [1], false)
			break;
		case "urso":
			if (crenca == "serAtacado")
				urso[id].serAtacado();
...
function estadoEmocional(agente) {
	var emocao
	switch (agente) {
		case "coelho":
			emocao = JSON.stringify(coelho[id].agente.getEmotionalState(true).sort(compare))
			break;
		case "urso":
			emocao = JSON.stringify(urso[id].agente.getEmotionalState(true).sort(compare))
			break;
		case "elefante":
			emocao = JSON.stringify(elefante[id].agente.getEmotionalState(true).sort(compare))
		default:
			break;
	}
	return emocao;
